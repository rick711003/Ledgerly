...
final class LedgerlyV1Tests: XCTestCase {
    override func tearDown() { UserDefaults.standard.removeObject(forKey: "appLanguage"); super.tearDown() }
    ...
    private final class ThrowingStore: LedgerStoring, @unchecked Sendable {
        ...
        func save(_ data: LedgerData) throws { if shouldFail { throw LedgerStoreError.retryable }; self.data = data }
        func clear() throws { if shouldFail { throw LedgerStoreError.retryable }; data = .empty }
    }
    private final class FailingStore: LedgerStoring, @unchecked Sendable {
        ...
        init(_ data: LedgerData = .empty) { self.data = data }
        func load() throws -> LedgerData { data }
        func save(_ data: LedgerData) throws { if shouldFailSave { throw LedgerStoreError.retryable }; self.data = data }
        func clear() throws { if shouldFailClear { throw LedgerStoreError.retryable }; data = .empty }
    }
    private func temporaryStore() -> FileLedgerStore {
        FileLedgerStore(url: FileManager.default.temporaryDirectory.appendingPathComponent(UUID().uuidString).appendingPathComponent("ledger.json"))
    }
    ...
    private func transaction(category: LedgerCategory, note: String = "") -> LedgerTransaction {
        let now = Date()
        return LedgerTransaction(id: UUID(), kind: .expense, amountMinor: 4260, currency: "USD", categoryID: category.id, categoryName: category.name, occurredOn: Calendar.current.dateComponents([.year, .month, .day], from: now), note: note, createdAt: now, updatedAt: now)
    }
    ...
    func testStoreRoundTripPreservesTransactionIdentity() throws {
        ...
        let ledger = LedgerData(formatVersion: LedgerData.formatVersion, currency: "USD", categories: [food], transactions: [entry])
    ...
    func testArchivedCategoryCannotValidateNewTransactionButHistoryRemains() throws {
        ...
        XCTAssertThrowsError(try LedgerValidator.validate(kind: .expense, amountMinor: 350, category: archived, date: Calendar.current.dateComponents([.year, .month, .day], from: Date()), note: "")) { error in
            XCTAssertEqual(error as? LedgerValidationError, .categoryUnavailable)
        }
        let ledger = LedgerData(formatVersion: LedgerData.formatVersion, currency: "USD", categories: [archived], transactions: [transaction(category: archived)])
        XCTAssertEqual(ledger.transactions.count, 1)
    }

    func testMonthlySummarySeparatesIncomeAndExpense() {
        let food = category()
        let incomeCategory = LedgerCategory(id: UUID(), kind: .income, name: "Salary", isBuiltIn: false, status: .active)
        let today = Calendar.current.dateComponents([.year, .month, .day], from: Date())
        let now = Date()
        let income = LedgerTransaction(id: UUID(), kind: .income, amountMinor: 10000, currency: "USD", categoryID: incomeCategory.id, categoryName: incomeCategory.name, occurredOn: today, note: "", createdAt: now, updatedAt: now)
        let expense = transaction(category: food)
        let ledger = LedgerData(formatVersion: LedgerData.formatVersion, currency: "USD", categories: [food, incomeCategory], transactions: [income, expense])
    ...
    func testCSVQuotesCommasAndQuotes() {
        let food = category()
        let ledger = LedgerData(formatVersion: LedgerData.formatVersion, currency: "USD", categories: [food], transactions: [transaction(category: food, note: "A, \"quoted\" note")])
        XCTAssertTrue(CSVExporter.makeCSV(ledger).contains("\"A, \"\"quoted\"\" note\""))
    }

    func testFutureDatesAreRejected() {
        let food = category()
        let future = Calendar.current.dateComponents([.year, .month, .day], from: Date().addingTimeInterval(86_400))
        XCTAssertThrowsError(try LedgerValidator.validate(kind: .expense, amountMinor: 100, category: food, date: future, note: "")) { error in
    ...
    func testTransactionOrderingUsesNewestDateThenCreationTime() {
        ...
        let older = LedgerTransaction(id: UUID(), kind: .expense, amountMinor: 100, currency: "USD", categoryID: food.id, categoryName: food.name, occurredOn: DateComponents(calendar: calendar, year: 2026, month: 7, day: 2), note: "older", createdAt: Date(timeIntervalSince1970: 1), updatedAt: Date(timeIntervalSince1970: 1))
        let newerSameDay = LedgerTransaction(id: UUID(), kind: .expense, amountMinor: 200, currency: "USD", categoryID: food.id, categoryName: food.name, occurredOn: DateComponents(calendar: calendar, year: 2026, month: 7, day: 2), note: "newer", createdAt: Date(timeIntervalSince1970: 2), updatedAt: Date(timeIntervalSince1970: 2))
        let latestDay = LedgerTransaction(id: UUID(), kind: .expense, amountMinor: 300, currency: "USD", categoryID: food.id, categoryName: food.name, occurredOn: DateComponents(calendar: calendar, year: 2026, month: 7, day: 3), note: "latest", createdAt: Date(timeIntervalSince1970: 0), updatedAt: Date(timeIntervalSince1970: 0))
        let ledger = LedgerData(formatVersion: LedgerData.formatVersion, currency: "USD", categories: [food], transactions: [older, latestDay, newerSameDay])

        XCTAssertEqual(ledger.transactions(in: month).map(\.id), [latestDay.id, newerSameDay.id, older.id])
    }

    func testOverlongNotesAreRejected() {
        let food = category()
        let today = Calendar.current.dateComponents([.year, .month, .day], from: Date())
        XCTAssertThrowsError(try LedgerValidator.validate(kind: .expense, amountMinor: 100, category: food, date: today, note: String(repeating: "x", count: 501))) { error in
    ...
    func testNewerFormatIsReportedAndItsBytesArePreserved() throws {
        let url = FileManager.default.temporaryDirectory.appendingPathComponent(UUID().uuidString).appendingPathComponent("ledger.json")
        try FileManager.default.createDirectory(at: url.deletingLastPathComponent(), withIntermediateDirectories: true)
        let original = try JSONEncoder().encode(LedgerData(formatVersion: LedgerData.formatVersion + 1, currency: "USD", categories: [], transactions: []))
        try original.write(to: url)
    ...
    func testFailedClearRetainsPublishedLedgerUntilStoreSucceeds() {
        ...
        let initial = LedgerData(formatVersion: LedgerData.formatVersion, currency: "USD", categories: [category], transactions: [transaction(category: category)])
    ...
    func testCurrencyCannotChangeAfterFirstTransactionAndTWDUsesZeroMinorDigits() {
        let category = category()
        let ledger = LedgerData(formatVersion: LedgerData.formatVersion, currency: "TWD", categories: [category], transactions: [transaction(category: category)])
        let store = FailingStore(ledger); store.shouldFailSave = false
        let model = LedgerViewModel(store: store)
    ...
    func testFailedDeleteAndClearLeavePublishedLedgerUnchanged() {
        ...
        let initial = LedgerData(formatVersion: LedgerData.formatVersion, currency: "USD", categories: [food], transactions: [record])
        let store = ThrowingStore(initial)
        let model = LedgerViewModel(store: store)
        XCTAssertFalse(model.mutate(success: .transactionDeleted, failure: .transactionDeleteFailed) { $0.transactions.removeAll() })
    ...
    func testTWDUsesZeroFractionDigits() {
        XCTAssertEqual(money(42, currency: "TWD"), money(42, currency: "TWD").replacingOccurrences(of: ".00", with: ""))
        XCTAssertEqual(amountText(42, currency: "TWD"), "42")
    }

    @MainActor
    func testFailedSetupDoesNotPublishAndCanRetry() {
        let store = FailingStore(); let model = LedgerViewModel(store: store)
        XCTAssertFalse(model.finishSetup(currency: "USD"))
        XCTAssertNil(model.ledger.currency)
    ...
    func testFailedTransactionMutationDoesNotPublish() {
        let store = FailingStore(); let model = LedgerViewModel(store: store); let item = transaction(category: category())
        XCTAssertFalse(model.mutate(success: .transactionSaved, failure: .transactionSaveFailed) { $0.transactions.append(item) })
        XCTAssertTrue(model.ledger.transactions.isEmpty)
        XCTAssertTrue(store.data.transactions.isEmpty)
    }
    ...
    func testFailedClearRetainsPublishedLedger() {
        let store = FailingStore(); store.shouldFailSave = false
        let model = LedgerViewModel(store: store)
        XCTAssertTrue(model.finishSetup(currency: "USD"))
    ...
    func testLocalizedCurrencyParsingAndPresentationDoNotChangeMinorUnits() {
        XCTAssertEqual(minorAmount("1,234.50", currency: "USD", locale: Locale(identifier: "en_US")), 123_450)
        XCTAssertEqual(minorAmount("1,234.50", currency: "USD", locale: Locale(identifier: "zh_Hant_TW")), 123_450)
        XCTAssertEqual(minorAmount("1,234", currency: "TWD", locale: Locale(identifier: "zh_Hant_TW")), 1_234)
        XCTAssertEqual(amountText(123_450, currency: "USD", locale: Locale(identifier: "en_US")), "1,234.5")
    }
    ...
    func testExplicitLanguageResolvesTraditionalChineseWithoutMutatingBuiltInData() {
        ...
        let stored = LedgerCategory(id: UUID(), kind: .expense, name: "Food", isBuiltIn: true, status: .active)
