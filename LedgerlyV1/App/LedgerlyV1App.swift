...
struct LedgerlyV1App: App {
    ...
    var body: some Scene { WindowGroup { RootView().environmentObject(model).environment(\.locale, (AppLanguage(rawValue: language) ?? .system).locale) } }
}

@MainActor
final class LedgerViewModel: ObservableObject {
    enum Recovery: Equatable { case retryable, integrity, unsupportedFormat }
    enum State: Equatable { case loading, onboarding, ready, recovery(Recovery) }
    ...
    init(store: LedgerStoring = FileLedgerStore()) { self.store = store; open() }

    func open() {
        do { ledger = try store.load(); state = ledger.currency == nil ? .onboarding : .ready }
        catch let error as LedgerStoreError {
            state = .recovery(error == .retryable ? .retryable : error == .integrity ? .integrity : .unsupportedFormat)
    ...
    @discardableResult
    func mutate(success: L10n.Key, failure: L10n.Key, _ change: (inout LedgerData) throws -> Void) -> Bool {
...
private extension Bool { func also(_ action: (Bool) -> Void) -> Bool { action(self); return self } }

enum Defaults { static var categories: [LedgerCategory] { [
    .init(id: UUID(), kind: .expense, name: "Food", isBuiltIn: true, status: .active), .init(id: UUID(), kind: .expense, name: "Transport", isBuiltIn: true, status: .active), .init(id: UUID(), kind: .expense, name: "Home", isBuiltIn: true, status: .active), .init(id: UUID(), kind: .income, name: "Salary", isBuiltIn: true, status: .active), .init(id: UUID(), kind: .income, name: "Freelance", isBuiltIn: true, status: .active)
] } }
