...
public struct LedgerData: Codable, Sendable {
  ...
  public static let empty = LedgerData(
    formatVersion: formatVersion, currency: nil, categories: [], transactions: [])
}
...
public enum LedgerValidator {
  public static func validate(
    kind: TransactionKind, amountMinor: Int64, category: LedgerCategory?, date: DateComponents,
    note: String, calendar: Calendar = .current
  ) throws {
    guard amountMinor > 0 else { throw LedgerValidationError.invalidAmount }
    guard let category, category.status == .active, category.kind == kind else {
      throw LedgerValidationError.categoryUnavailable
    }
    guard note.count <= 500 else { throw LedgerValidationError.noteTooLong }
    let today = calendar.dateComponents([.year, .month, .day], from: Date())
    guard let selected = calendar.date(from: date), let current = calendar.date(from: today),
      selected <= current
    else { throw LedgerValidationError.futureDate }
  }
  public static func normalizedCategoryName(_ value: String) -> String {
...
extension LedgerData {
  public func transactions(in month: DateComponents) -> [LedgerTransaction] {
  ...
  public func summary(in month: DateComponents) -> MonthlySummary {
    transactions(in: month).reduce(MonthlySummary(income: 0, expense: 0)) { result, transaction in
      var copy = result
      if transaction.kind == .income {
        copy.income += transaction.amountMinor
      } else {
        copy.expense += transaction.amountMinor
      }
      return copy
    }
  }
